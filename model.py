...
class UnetAux_v2(SegmentationModel):
    ...
    def forward(self, x):
        ...
        # print(list(map(lambda x: x.shape, decoder_outputs)))
        seg0 = self.segmentation_head(decoder_outputs[0])
        seg1 = self.segmentation_head(decoder_outputs[1])
        seg2 = self.segmentation_head(decoder_outputs[2])
        if self.training:
            return seg0,seg1,seg2
        return seg0
...
if __name__ == '__main__':
    ...
    model = UnetAux_v2(encoder_name='dpn68', classes=2, encoder_depth=5, decoder_channels=(256, 128, 64, 32, 16), decoder_attention_type='scse')
