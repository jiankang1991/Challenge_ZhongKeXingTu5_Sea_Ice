...
from albumentations.augmentations.transforms import   Normalize, HueSaturationValue, RGBShift, RandomBrightnessContrast,        \
    GaussNoise, CLAHE, RandomGamma, ToFloat,    \
    RandomFog
from albumentations.core.composition import Compose, OneOf, OneOrOther
from albumentations.augmentations.crops.transforms import Crop, CenterCrop, RandomCrop, RandomResizedCrop, RandomSizedCrop
from albumentations.augmentations.geometric.resize import Resize
from albumentations.augmentations.geometric.transforms import ElasticTransform, ShiftScaleRotate, HorizontalFlip, VerticalFlip
...
__all__ = ['Crop', 'VerticalFlip', 'HorizontalFlip', 'Flip', 'Transpose',
           ...
           'RandomResizedCrop', 'RandomFog', 'ShiftScaleRotate']
...
def _check_augs(augs, additional_targets=None):
    """Check if augmentations are loaded in already or not."""
    if isinstance(augs, dict):
        return process_aug_dict(augs, additional_targets=additional_targets)
    elif isinstance(augs, Compose):
        return augs
    


def process_aug_dict(pipeline_dict, meta_augs_list=['oneof', 'oneorother'], additional_targets=None):
    ...
    return Compose(composer_list, p=p, additional_targets=additional_targets)
...
aug_matcher = {
    ...
    'randomresizedcrop': RandomResizedCrop,  'elastictransform': ElasticTransform,
   ...
   'oneof': OneOf, 'oneorother': OneOrOther, 'noop': NoOp,
    'randomrotate90': RandomRotate90, 'dropchannel': DropChannel,
    'swapchannels': SwapChannels,  
    'RandomFog'.lower():RandomFog, 'ShiftScaleRotate'.lower():ShiftScaleRotate
}
